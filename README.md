...
> **Примечание по логике подсчёта:** эндпоинт возвращает сумму месячных цен всех подписок,
> которые были активны хотя бы частично в указанном периоде. Подписка считается активной
> в периоде если она началась до конца периода и либо не имеет даты окончания, либо
> заканчивается после начала периода. Стоимость каждой подписки берётся как фиксированная
> месячная цена без умножения на количество месяцев пересечения — суммируется факт наличия
> активной подписки, а не её длительность.
